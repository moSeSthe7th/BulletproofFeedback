...
public class PlatformTurnScript : Rotatable {

    public static PlatformTurnScript instance;
    ...
    private void Start()
    {
        ...
        //BlockRot.Add(leftUpR);
        bullet.transform.position = new Vector3(0f, Player.instance.transform.position.y, -30f);
        ...
        //Vector3 coinPosMiddleUp = new Vector3(0f, -Player.instance.transform.position.y, 0f);
    ...
    public void SwipePlatform(int dirr)
    {
        ...
        if (dirr == (int)InputManager.direction.left && !changing && angle < 60f && Player.instance.state != (int)Player.State.down)//angle <= 0f)
        {
            changing = true;
            Player.instance.SetPlayerRight();
            SwipePlatformCs = StartCoroutine(PlatformTurner(dirr,currentLocation,turnSpeed));
        }
        else if (dirr == (int)InputManager.direction.right && !changing && angle > -60f && Player.instance.state != (int)Player.State.down)//angle >= 0f)
        {
            changing = true;
            Player.instance.SetPlayerLeft();
            SwipePlatformCs = StartCoroutine(PlatformTurner(dirr,currentLocation,turnSpeed));
        }
        else
